...
export function MagneticText({ text = "CREATIVE", hoverText = "EXPLORE", className }: MagneticTextProps) {
  ...
  const hoverLines = (hoverText || text).split("\n")
    ...
    >
      ...
      <div className="flex flex-col items-center leading-[0.85]">
        {lines.map((line, i) => (
          <span 
            key={i} 
            className="text-8xl font-black text-[#002855] tracking-tighter whitespace-nowrap uppercase font-[family-name:var(--font-outfit)]"
          >
            {line}
          </span>
        ))}
      ...
      <div
        ref={circleRef}
        className="absolute top-0 left-0 pointer-events-none rounded-full bg-[#002855] overflow-hidden"
        style={{
      ...
      >
        <div
          ref={innerTextRef}
          className="absolute flex flex-col items-center justify-center leading-[0.85]"
          style={{
        ...
        >
          {hoverLines.map((line, i) => (
            <span 
              key={i} 
              className="text-8xl font-black text-white whitespace-nowrap tracking-tighter uppercase font-[family-name:var(--font-outfit)]"
            >
              {line}
            </span>
          ))}
